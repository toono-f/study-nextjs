...
const htmlReplace = () => {
  return gulp
    ...
    .pipe(replace(/<meta name="robots" content="index,follow"\/>/g, "")) // 不要記述を削除
    .pipe(replace(/<meta name="googlebot" content="index,follow"\/>/g, "")) // 不要記述を削除
    .pipe(replace(/charSet/g, "charset")) // 小文字変換（W3cエラー回避）
    .pipe(replace(/\/><meta charset="utf-8"\/>/g, "/>")) // 不要記述を削除
    .pipe(gulp.dest(`./${path}/`));
};
